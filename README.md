...
1. **下载脚本**：
   - 下载本项目中的 [obs-auto-zoom.lua](obs-auto-zoom.lua) 到您的本地目录。
2. **导入 OBS**：
   - 打开 OBS Studio，在上方菜单栏选择 `工具` -> `脚本`。
   - 在弹出的窗口中，点击左下角的 **`+`** 按钮，选择下载好的 `obs-auto-zoom.lua` 导入。
3. **选择缩放源**：
...
导入脚本后，请前往 OBS 的 **设置 -> 快捷键**，往下滚动或搜索 `obs-auto-zoom`，为以下功能绑定您顺手的快捷键：
